...
describe('ImportSqlDialog', () => {
  ...
  it('应在前端拦截超长 SQL 并提示错误', async () => {
    ...
    expect(
      await screen.findByText('SQL 内容过长，最大允许 50,000 个字符'),
    ).toBeInTheDocument();
    expect(mockedRequestSqlParse).not.toHaveBeenCalled();
  });

  it('应使用统一友好文案展示解析失败信息', async () => {
    mockedRequestSqlParse.mockRejectedValue(
      new Error('parser-stack-detail-should-not-be-exposed'),
    );
    render(
      <ImportSqlDialog
        currentDbType="mysql"
    ...
    const errorText = await screen.findByText(
      'SQL 解析失败，请检查 SQL 语法后重试。',
    );
    expect(errorText).toBeInTheDocument();
    expect(
      screen.queryByText('parser-stack-detail-should-not-be-exposed'),
    ).toBeNull();
  });

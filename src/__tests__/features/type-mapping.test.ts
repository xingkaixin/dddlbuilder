...
describe('Type Mapping Functions', () => {
  ...
  describe('canonicalizeBaseType', () => {
    ...
    it('should map complex type aliases', () => {
      ...
      expect(canonicalizeBaseType('national character varying')).toBe(
        'nvarchar',
      );
      expect(canonicalizeBaseType('double precision')).toBe('double');
      expect(canonicalizeBaseType('timestamp without time zone')).toBe(
        'timestamp',
      );
      expect(canonicalizeBaseType('timestamp with time zone')).toBe(
  ...
  describe('getFieldTypeForDatabase', () => {
    describe('MySQL type mapping', () => {
      it('should map varchar types', () => {
        expect(getFieldTypeForDatabase('mysql', 'varchar')).toBe(
          'VARCHAR(255)',
        );
        expect(getFieldTypeForDatabase('mysql', 'varchar(100)')).toBe(
          'VARCHAR(100)',
        );
        expect(getFieldTypeForDatabase('mysql', 'nvarchar')).toBe(
          'VARCHAR(255)',
        );
        expect(getFieldTypeForDatabase('mysql', 'nvarchar(50)')).toBe(
          'VARCHAR(50)',
        );
      });
      ...
      it('should map integer types', () => {
        ...
        expect(getFieldTypeForDatabase('mysql', 'int unsigned')).toBe(
          'INT UNSIGNED',
        );
        expect(getFieldTypeForDatabase('mysql', 'tinyint')).toBe('TINYINT(1)');
        expect(getFieldTypeForDatabase('mysql', 'tinyint(8)')).toBe(
          'TINYINT(8)',
        );
        expect(getFieldTypeForDatabase('mysql', 'tinyint unsigned')).toBe(
          'TINYINT(1) UNSIGNED',
        );
        expect(getFieldTypeForDatabase('mysql', 'smallint')).toBe('SMALLINT');
        expect(getFieldTypeForDatabase('mysql', 'bigint')).toBe('BIGINT');
        expect(getFieldTypeForDatabase('mysql', 'bigint unsigned')).toBe(
          'BIGINT UNSIGNED',
        );
      });

      it('should map decimal types', () => {
        expect(getFieldTypeForDatabase('mysql', 'decimal')).toBe(
          'DECIMAL(10, 3)',
        );
        expect(getFieldTypeForDatabase('mysql', 'decimal(10,3)')).toBe(
          'DECIMAL(10, 3)',
        );
        expect(getFieldTypeForDatabase('mysql', 'decimal(5)')).toBe(
          'DECIMAL(5)',
        );
      ...
      it('should map text types', () => {
        ...
        expect(getFieldTypeForDatabase('mysql', 'mediumtext')).toBe(
          'MEDIUMTEXT',
        );
      ...
      it('should map date/time types', () => {
        ...
        expect(getFieldTypeForDatabase('mysql', 'datetime(6)')).toBe(
          'DATETIME(6)',
        );
        expect(getFieldTypeForDatabase('mysql', 'timestamp')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('mysql', 'timestamp(6)')).toBe(
          'TIMESTAMP',
        );
      ...
      it('should map JSON and binary types', () => {
        ...
        expect(getFieldTypeForDatabase('mysql', 'varbinary(100)')).toBe(
          'VARBINARY(100)',
        );
        expect(getFieldTypeForDatabase('mysql', 'varbinary(max)')).toBe(
          'VARBINARY(MAX)',
        );
      });
      ...
      it('should map other common types', () => {
        ...
        expect(getFieldTypeForDatabase('mysql', 'serial')).toBe(
          'BIGINT UNSIGNED AUTO_INCREMENT',
        );
      });

      it('should return original type for unknown types', () => {
        expect(getFieldTypeForDatabase('mysql', 'unknown_type')).toBe(
          'unknown_type',
        );
    ...
    describe('PostgreSQL type mapping', () => {
      it('should map character types', () => {
        expect(getFieldTypeForDatabase('postgresql', 'varchar')).toBe(
          'VARCHAR',
        );
        expect(getFieldTypeForDatabase('postgresql', 'varchar(100)')).toBe(
          'VARCHAR(100)',
        );
        expect(getFieldTypeForDatabase('postgresql', 'char')).toBe('CHAR');
        expect(getFieldTypeForDatabase('postgresql', 'char(10)')).toBe(
          'CHAR(10)',
        );
        expect(getFieldTypeForDatabase('postgresql', 'nvarchar')).toBe(
          'VARCHAR',
        );
        expect(getFieldTypeForDatabase('postgresql', 'nchar')).toBe('CHAR');
        expect(getFieldTypeForDatabase('postgresql', 'character varying')).toBe(
          'VARCHAR',
        );
      });
      ...
      it('should map integer types', () => {
        ...
        expect(getFieldTypeForDatabase('postgresql', 'integer')).toBe(
          'INTEGER',
        );
        expect(getFieldTypeForDatabase('postgresql', 'tinyint')).toBe(
          'SMALLINT',
        );
        expect(getFieldTypeForDatabase('postgresql', 'smallint')).toBe(
          'SMALLINT',
        );
      ...
      it('should map decimal types', () => {
        expect(getFieldTypeForDatabase('postgresql', 'decimal')).toBe(
          'NUMERIC(10, 3)',
        );
        expect(getFieldTypeForDatabase('postgresql', 'decimal(10,3)')).toBe(
          'NUMERIC(10, 3)',
        );
        expect(getFieldTypeForDatabase('postgresql', 'number')).toBe(
          'NUMERIC(10, 3)',
        );
        expect(getFieldTypeForDatabase('postgresql', 'numeric')).toBe(
          'NUMERIC(10, 3)',
        );
      });
      ...
      it('should map date/time types', () => {
        ...
        expect(getFieldTypeForDatabase('postgresql', 'time')).toBe(
          'TIME WITHOUT TIME ZONE',
        );
        expect(getFieldTypeForDatabase('postgresql', 'timetz')).toBe(
          'TIME WITH TIME ZONE',
        );
        expect(getFieldTypeForDatabase('postgresql', 'timestamp')).toBe(
          'TIMESTAMP',
        );
        expect(getFieldTypeForDatabase('postgresql', 'timestamptz')).toBe(
          'TIMESTAMP WITH TIME ZONE',
        );
        expect(getFieldTypeForDatabase('postgresql', 'datetime')).toBe(
          'TIMESTAMP',
        );
        expect(
          getFieldTypeForDatabase('postgresql', 'time with time zone'),
        ).toBe('TIME WITH TIME ZONE');
      });

      it('should map text types', () => {
        expect(getFieldTypeForDatabase('postgresql', 'text')).toBe('TEXT');
        expect(getFieldTypeForDatabase('postgresql', 'mediumtext')).toBe(
          'TEXT',
        );
      ...
      it('should map other types', () => {
        expect(getFieldTypeForDatabase('postgresql', 'boolean')).toBe(
          'BOOLEAN',
        );
        expect(getFieldTypeForDatabase('postgresql', 'bit')).toBe('BOOLEAN');
        expect(getFieldTypeForDatabase('postgresql', 'uuid')).toBe('UUID');
        expect(getFieldTypeForDatabase('postgresql', 'float')).toBe(
          'DOUBLE PRECISION',
        );
        expect(getFieldTypeForDatabase('postgresql', 'double')).toBe(
          'DOUBLE PRECISION',
        );
    ...
    describe('SQL Server type mapping', () => {
      it('should map character types', () => {
        expect(getFieldTypeForDatabase('sqlserver', 'varchar')).toBe(
          'VARCHAR(255)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'varchar(100)')).toBe(
          'VARCHAR(100)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'nvarchar')).toBe(
          'NVARCHAR(255)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'nvarchar(100)')).toBe(
          'NVARCHAR(100)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'char')).toBe('CHAR(1)');
        expect(getFieldTypeForDatabase('sqlserver', 'char(10)')).toBe(
          'CHAR(10)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'nchar')).toBe('NCHAR(1)');
        expect(getFieldTypeForDatabase('sqlserver', 'nchar(10)')).toBe(
          'NCHAR(10)',
        );
      });

      it('should map text types', () => {
        expect(getFieldTypeForDatabase('sqlserver', 'text')).toBe(
          'NVARCHAR(MAX)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'mediumtext')).toBe(
          'NVARCHAR(MAX)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'longtext')).toBe(
          'NVARCHAR(MAX)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'clob')).toBe(
          'NVARCHAR(MAX)',
        );
      });
      ...
      it('should map integer types', () => {
        ...
        expect(getFieldTypeForDatabase('sqlserver', 'smallint')).toBe(
          'SMALLINT',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'bigint')).toBe('BIGINT');
        expect(getFieldTypeForDatabase('sqlserver', 'serial')).toBe(
          'BIGINT IDENTITY(1,1)',
        );
      });
      ...
      it('should map date/time types', () => {
        ...
        expect(getFieldTypeForDatabase('sqlserver', 'datetime')).toBe(
          'DATETIME2',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'datetime2')).toBe(
          'DATETIME2',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'datetime(6)')).toBe(
          'DATETIME2(6)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'timestamp')).toBe(
          'DATETIME2',
        );
      ...
      it('should map other types', () => {
        ...
        expect(getFieldTypeForDatabase('sqlserver', 'uuid')).toBe(
          'UNIQUEIDENTIFIER',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'varbinary')).toBe(
          'VARBINARY(MAX)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'varbinary(100)')).toBe(
          'VARBINARY(100)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'xml')).toBe('XML');
        expect(getFieldTypeForDatabase('sqlserver', 'json')).toBe(
          'NVARCHAR(MAX)',
        );
        expect(getFieldTypeForDatabase('sqlserver', 'jsonb')).toBe(
          'NVARCHAR(MAX)',
        );
      });
    });
    ...
    describe('Oracle type mapping', () => {
      it('should map character types', () => {
        expect(getFieldTypeForDatabase('oracle', 'varchar')).toBe(
          'VARCHAR2(100)',
        );
        expect(getFieldTypeForDatabase('oracle', 'varchar(100)')).toBe(
          'VARCHAR2(100)',
        );
        expect(getFieldTypeForDatabase('oracle', 'nvarchar')).toBe(
          'NVARCHAR2(100)',
        );
        expect(getFieldTypeForDatabase('oracle', 'nvarchar(100)')).toBe(
          'NVARCHAR2(100)',
        );
        ...
        expect(getFieldTypeForDatabase('oracle', 'nchar(10)')).toBe(
          'NCHAR(10)',
        );
        expect(getFieldTypeForDatabase('oracle', 'varchar2')).toBe(
          'VARCHAR2(100)',
        );
        expect(getFieldTypeForDatabase('oracle', 'nvarchar2')).toBe(
          'NVARCHAR2(100)',
        );
      });
      ...
      it('should map decimal types', () => {
        expect(getFieldTypeForDatabase('oracle', 'decimal')).toBe(
          'NUMBER(10, 2)',
        );
        expect(getFieldTypeForDatabase('oracle', 'decimal(10,3)')).toBe(
          'NUMBER(10, 3)',
        );
        expect(getFieldTypeForDatabase('oracle', 'number')).toBe(
          'NUMBER(10, 2)',
        );
        expect(getFieldTypeForDatabase('oracle', 'numeric')).toBe(
          'NUMBER(10, 2)',
        );
      });
      ...
      it('should map date/time types', () => {
        ...
        expect(getFieldTypeForDatabase('oracle', 'datetime2')).toBe(
          'TIMESTAMP',
        );
        expect(getFieldTypeForDatabase('oracle', 'timestamp')).toBe(
          'TIMESTAMP',
        );
        expect(getFieldTypeForDatabase('oracle', 'timestamptz')).toBe(
          'TIMESTAMP WITH TIME ZONE',
        );
      ...
      it('should map other types', () => {
        ...
        expect(getFieldTypeForDatabase('oracle', 'double')).toBe(
          'BINARY_DOUBLE',
        );
        ...
        expect(getFieldTypeForDatabase('oracle', 'varbinary(100)')).toBe(
          'RAW(100)',
        );

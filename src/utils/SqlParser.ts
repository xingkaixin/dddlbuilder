...
export class SqlParser {
  ...
  private preprocessOracle(sql: string) {
    ...
    // 提取并移除 COMMENT 语句
    sql = sql.replace(/COMMENT\s+ON\s+TABLE\s+([\w".]+)\s+IS\s+'([^']*)'\s*;/gi, (_m, _table, comment) => {
      tableComment = comment;
      return "";
    });

    sql = sql.replace(/COMMENT\s+ON\s+COLUMN\s+([\w".]+)\.([\w"]+)\s+IS\s+'([^']*)'\s*;/gi, (_m, _table, column, comment) => {
  ...
  private extractStandaloneComments(sql: string) {
    ...
    const cleanedSql = sql
      .replace(/COMMENT\s+ON\s+TABLE\s+([\w".]+)\s+IS\s+'([^']*)'\s*;/gi, (_m, _table, comment) => {
        tableComment = comment;
        return "";
      })
      .replace(/COMMENT\s+ON\s+COLUMN\s+([\w".]+)\.([\w"]+)\s+IS\s+'([^']*)'\s*;/gi, (_m, _table, column, comment) => {
        const colName = column.replace(/"/g, "");
        columnComments[colName] = comment;
        return "";
      });
  ...
  private mergeComments(result: ParsedResult, tableComment: string, columnComments: Record<string, string>) {
  ...
  private buildTypeString(definition: any): string {
    ...
    if (Array.isArray(definition?.suffix) && definition.suffix.length > 0) {
      const suffixValues = definition.suffix
        .filter((v: any) => v !== null && v !== undefined && String(v).toLowerCase() !== "null");
  ...
  private buildIndexFields(columns: any[]): IndexField[] {
    ...
    return columns
      .map((col: any) => {
        ...
        if (!name) return null;
        const direction = (col?.order_by || col?.order_by_expr || col?.order)
  ...
  parse(sql: string, dbType: DatabaseType): ParsedResult {
    let sqlToParse = sql;
    let extractedComments: { tableComment: string; columnComments: Record<string, string> } | null = null;
    let rawGrantUsers: string[] = [];

    const mergeCommentSource = (source: { tableComment: string; columnComments: Record<string, string> } | null) => {
      if (!source) return;
      if (!extractedComments) {
        extractedComments = { tableComment: source.tableComment, columnComments: { ...source.columnComments } };
        return;
      }
    ...
    const opt = {
      database: dbType === 'sqlserver' ? 'transactsql' : dbType === 'oracle' ? 'mysql' : dbType,
    };
    ...
    if (stmt.create_definitions) {
      stmt.create_definitions.forEach((def: any) => {
        if (def.resource === 'column') {
          ...
          if (def.primary_key) {
             this.pushIndex(result, 'PRIMARY', [{ name: field.name, direction: 'ASC' }], true, true);
          ...
          if (def.unique) {
            ...
            if (def.constraint_type === 'primary key') {
                 ...
                 this.enforceNotNullForFields(result, fields.map((f) => f.name));
            } else if (def.constraint_type === 'unique key' || def.constraint_type === 'unique') {
                 const fields = this.buildIndexFields(def.definition || []);
                 const indexName = def.constraint || def.index || `uk_${fields.map((f:any)=>f.name).join('_')}`;
  ...
  private mapColumnToField(colDef: any, _dbType: DatabaseType): NormalizedField {
    ...
    const onUpdateSource = colDef.on_update?.value || colDef.on_update || (colDef.default_val?.value && typeof colDef.default_val.value === 'object' ? colDef.default_val.value.over : undefined);

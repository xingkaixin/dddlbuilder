...
function isTypeAssertion(node: ESTree.Node | null | undefined): node is TypeAssertion {
  return node?.type === "TSAsExpression" || node?.type === "TSTypeAssertion";
}

function isInnerAssertionInNonConstChain(node: TypeAssertion): boolean {
  const parent = node.parent;
  return isTypeAssertion(parent) && parent.expression === node && !isConstAssertion(parent);
}
...
/** Require the outermost non-const type assertion to state the invariant TypeScript cannot express. */
export const requireSafetyCommentForTypeAssertionRule = defineRule({
  meta: {
    type: "problem",
    docs: {
      description:
        "Require a nearby SAFETY comment for every outermost non-const TypeScript type assertion.",
    },
    messages: {
  ...
  createOnce(context) {
    ...
    const checkAssertion = (node: TypeAssertion) => {
      if (isConstAssertion(node) || isInnerAssertionInNonConstChain(node)) return;

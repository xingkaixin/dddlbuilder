...
import { generateAlterDDL, generateRollbackDDL, hasTableChanges } from '@ddlbuilder/ddl-core';
...
const FieldDiffRow = memo<{ diff: FieldDiff }>(({ diff }) => {
  ...
  const oldField = diff.type === 'add' ? undefined : diff.oldField;
 ...
 * Diff 对话框组件
 */
export const DiffDialog = memo<DiffDialogProps>(({ open, onOpenChange, diff, onCopy }) => {
  const { t } = useTranslation();
                      ...
                      {diff.oldMiscConfig.enabled && diff.newMiscConfig.enabled && (
                        <>
                          ...
                          {diff.oldMiscConfig.fillfactor !== diff.newMiscConfig.fillfactor && (
                            <div>
                              FILLFACTOR: {diff.oldMiscConfig.fillfactor ?? t('diffDialog.default')}
                              <span className="mx-1">→</span>
          ...
          {alterDDL && (
            <div className="space-y-2">
              ...
              <div className="max-h-48 overflow-auto rounded-md border bg-muted/30">
                <Suspense
                  fallback={
                    <pre className="m-0 whitespace-pre-wrap p-3 font-mono text-xs">{alterDDL}</pre>
                  }
                >

...
<script>const search=document.getElementById('search');search.addEventListener('input',()=>{const query=search.value.toLocaleLowerCase();document.querySelectorAll('main section').forEach(section=>{section.hidden=!section.textContent.toLocaleLowerCase().includes(query)})});document.querySelector('nav').addEventListener('click',()=>{search.value='';document.querySelectorAll('main section').forEach(section=>{section.hidden=false})});</script></body></html>`;
}

...
import { mergeLocalDraftChanges } from '@/hooks/workspacePersistence/mergeLocalEdits';
...
describe('mergeLocalDraftChanges', () => {
  ...
  it('两端改同一个键时远端赢', () => {
    ...
    expect(mergeLocalDraftChanges(base, local, remote).tableName).toBe('remote');
  });
  ...
  it('保留本地对索引等结构化字段的修改', () => {
    ...
    expect(mergeLocalDraftChanges(base, local, remote).indexes).toEqual(indexes);
  });

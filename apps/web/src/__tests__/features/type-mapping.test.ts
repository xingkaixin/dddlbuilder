  ...
  describe('getFieldTypeForDatabase', () => {
    describe('MySQL type mapping', () => {
      ...
      it('should map decimal types', () => {
        ...
        expect(getFieldTypeForDatabase('mysql', 'decimal unsigned')).toBe(
    ...
    describe('PostgreSQL type mapping', () => {
      ...
      it('should map date/time types', () => {
        ...
        expect(getFieldTypeForDatabase('postgresql', 'timestamptz')).toBe(
          'TIMESTAMP WITH TIME ZONE',
        );
        expect(getFieldTypeForDatabase('postgresql', 'datetime')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('postgresql', 'time with time zone')).toBe(
          'TIME WITH TIME ZONE',
        );
      });
    ...
    describe('Oracle type mapping', () => {
      ...
      it('should map integer types', () => {
        ...
        expect(getFieldTypeForDatabase('oracle', 'serial')).toBe(
    ...
    describe('MariaDB type mapping', () => {
      it('should behave the same as MySQL', () => {
        ...
        expect(getFieldTypeForDatabase('mariadb', 'varchar')).toBe('VARCHAR(255)');
        expect(getFieldTypeForDatabase('mariadb', 'int')).toBe('INT');
        expect(getFieldTypeForDatabase('mariadb', 'int unsigned')).toBe('INT UNSIGNED');
        expect(getFieldTypeForDatabase('mariadb', 'timestamp')).toBe('TIMESTAMP');
    ...
    describe('TiDB type mapping', () => {
      it('should behave the same as MySQL', () => {
        ...
        expect(getFieldTypeForDatabase('tidb', 'int unsigned')).toBe('INT UNSIGNED');
    ...
    describe('OceanBase MySQL mode type mapping', () => {
      it('should behave the same as MySQL', () => {
        ...
        expect(getFieldTypeForDatabase('oceanbase', 'varchar')).toBe('VARCHAR(255)');
        expect(getFieldTypeForDatabase('oceanbase', 'int')).toBe('INT');
        expect(getFieldTypeForDatabase('oceanbase', 'int unsigned')).toBe('INT UNSIGNED');
        expect(getFieldTypeForDatabase('oceanbase', 'timestamp')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('oceanbase', 'datetime')).toBe('DATETIME');
    ...
    describe('OceanBase Oracle mode type mapping', () => {
      it('should behave the same as Oracle', () => {
        ...
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'varchar')).toBe('VARCHAR2(100)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'int')).toBe('NUMBER(10)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'decimal(10,3)')).toBe('NUMBER(10, 3)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'timestamp')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'date')).toBe('DATE');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'text')).toBe('CLOB');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'json')).toBe('CLOB');
      });

      it('should map all character types correctly', () => {
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'char')).toBe('CHAR(1)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'char(10)')).toBe('CHAR(10)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'nchar')).toBe('NCHAR(1)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'nvarchar')).toBe('NVARCHAR2(100)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'nvarchar(50)')).toBe('NVARCHAR2(50)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'varchar2')).toBe('VARCHAR2(100)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'nvarchar2')).toBe('NVARCHAR2(100)');
      });

      it('should map all integer types correctly', () => {
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'integer')).toBe('NUMBER(10)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'smallint')).toBe('NUMBER(5)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'tinyint')).toBe('NUMBER(3)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'bigint')).toBe('NUMBER(19)');
      });

      it('should map all decimal types correctly', () => {
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'decimal')).toBe('NUMBER(10, 2)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'numeric')).toBe('NUMBER(10, 2)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'number')).toBe('NUMBER(10, 2)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'number(15,4)')).toBe('NUMBER(15, 4)');
      });

      it('should map all date/time types correctly', () => {
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'datetime')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'datetime2')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'timestamptz')).toBe(
          'TIMESTAMP WITH TIME ZONE',
        );
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'time')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'timetz')).toBe('TIMESTAMP');
      });

      it('should map LOB and binary types correctly', () => {
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'clob')).toBe('CLOB');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'blob')).toBe('BLOB');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'varbinary')).toBe('RAW(100)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'varbinary(200)')).toBe('RAW(200)');
      });

      it('should map other types correctly', () => {
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'boolean')).toBe('NUMBER(1)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'bit')).toBe('NUMBER(1)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'uuid')).toBe('CHAR(36)');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'xml')).toBe('XMLTYPE');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'float')).toBe('FLOAT');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'double')).toBe('BINARY_DOUBLE');
        expect(getFieldTypeForDatabase('oceanbase-oracle', 'real')).toBe('BINARY_FLOAT');
      });
    ...
    describe('PostgreSQL Citus type mapping', () => {
      it('should reuse PostgreSQL type mappings', () => {
        ...
        expect(getFieldTypeForDatabase('postgresql-citus', 'varchar')).toBe('VARCHAR');
        expect(getFieldTypeForDatabase('postgresql-citus', 'int')).toBe('INTEGER');
        expect(getFieldTypeForDatabase('postgresql-citus', 'decimal(10,2)')).toBe('NUMERIC(10, 2)');
        expect(getFieldTypeForDatabase('postgresql-citus', 'timestamp')).toBe('TIMESTAMP');
        expect(getFieldTypeForDatabase('postgresql-citus', 'json')).toBe('JSONB');
        expect(getFieldTypeForDatabase('postgresql-citus', 'text')).toBe('TEXT');
      });

      it('should map character types correctly', () => {
        expect(getFieldTypeForDatabase('postgresql-citus', 'varchar(100)')).toBe('VARCHAR(100)');
        expect(getFieldTypeForDatabase('postgresql-citus', 'char')).toBe('CHAR');
        expect(getFieldTypeForDatabase('postgresql-citus', 'char(10)')).toBe('CHAR(10)');
        expect(getFieldTypeForDatabase('postgresql-citus', 'nvarchar')).toBe('VARCHAR');
        expect(getFieldTypeForDatabase('postgresql-citus', 'character varying')).toBe('VARCHAR');
      });

      it('should map integer types correctly', () => {
        expect(getFieldTypeForDatabase('postgresql-citus', 'integer')).toBe('INTEGER');
        expect(getFieldTypeForDatabase('postgresql-citus', 'smallint')).toBe('SMALLINT');
        expect(getFieldTypeForDatabase('postgresql-citus', 'bigint')).toBe('BIGINT');
        expect(getFieldTypeForDatabase('postgresql-citus', 'serial')).toBe('SERIAL');
        expect(getFieldTypeForDatabase('postgresql-citus', 'bigserial')).toBe('BIGSERIAL');
      });

      it('should map date/time types correctly', () => {
        expect(getFieldTypeForDatabase('postgresql-citus', 'date')).toBe('DATE');
        expect(getFieldTypeForDatabase('postgresql-citus', 'time')).toBe('TIME WITHOUT TIME ZONE');
        expect(getFieldTypeForDatabase('postgresql-citus', 'timetz')).toBe('TIME WITH TIME ZONE');
        expect(getFieldTypeForDatabase('postgresql-citus', 'timestamptz')).toBe(
          'TIMESTAMP WITH TIME ZONE',
        );
        expect(getFieldTypeForDatabase('postgresql-citus', 'datetime')).toBe('TIMESTAMP');
      });

      it('should map JSON and binary types correctly', () => {
        expect(getFieldTypeForDatabase('postgresql-citus', 'json')).toBe('JSONB');
        expect(getFieldTypeForDatabase('postgresql-citus', 'jsonb')).toBe('JSONB');
        expect(getFieldTypeForDatabase('postgresql-citus', 'blob')).toBe('BYTEA');
        expect(getFieldTypeForDatabase('postgresql-citus', 'varbinary')).toBe('BYTEA');
        expect(getFieldTypeForDatabase('postgresql-citus', 'xml')).toBe('XML');
      });

      it('should map other types correctly', () => {
        expect(getFieldTypeForDatabase('postgresql-citus', 'boolean')).toBe('BOOLEAN');
        expect(getFieldTypeForDatabase('postgresql-citus', 'bit')).toBe('BOOLEAN');
        expect(getFieldTypeForDatabase('postgresql-citus', 'uuid')).toBe('UUID');
        expect(getFieldTypeForDatabase('postgresql-citus', 'float')).toBe('DOUBLE PRECISION');
        expect(getFieldTypeForDatabase('postgresql-citus', 'double')).toBe('DOUBLE PRECISION');
        expect(getFieldTypeForDatabase('postgresql-citus', 'real')).toBe('REAL');
      });
    });
  });
});

export const zhCNCommon = {
  ...
  header: {
    ...
    auth: {
      ...
      dialogDescriptionSignUp: '创建账号后，输入邮件中的六位验证码完成邮箱验证。',
      ...
      sendVerificationCode: '验证邮箱',
      verifyCodeTitle: '验证你的邮箱',
      verifyCodeDescription: '六位验证码已发送至 {{email}}，请在下方输入。',
      otpLabel: '六位验证码',
      otpHint: '验证码 10 分钟内有效。未收到时，请检查垃圾邮件。',
      verifyCode: '验证并登录',
      resendCodeIn: '{{count}} 秒后可重发',
      sendCodeFailed: '验证码发送失败，请稍后重试',
      verifyCodeFailed: '验证失败，请重试',
      invalidOtp: '验证码不正确，请重新输入',
      otpExpired: '验证码已过期，请重新发送',
      otpAttemptsExceeded: '尝试次数过多，请重新发送验证码',
      otpVerified: '邮箱验证成功，已登录',
  ...
  tableTemplate: {
    ...
    preview: {
      ...
      indexList: '索引预览',
      excluded:
        '蓝本保存字段、索引、数据库类型和表级配置；保留当前表名、表注释和授权对象，不继承外键。应用后请为新表配置外键。',
      cancel: '取消',
      apply: '应用蓝本',
    },
    create: {
  ...
  savedTables: {
    ...
    toast: {
      ...
      workspaceNotReady: '工作区未就绪',
      tableNotFound: '未找到保存的表',
      loadFailed: '加载失败',
      ...
      restoreFailed: '恢复失败',
      renameFailed: '重命名失败',
      noChangesToSave: '加载的表未修改，无法保存',
      nameExists: '名称已存在，请换一个',
      tableSaved: '已保存：{{name}}',
      tableUpdated: '已更新：{{name}}',
      tableRenamed: '已重命名为：{{name}}',
      tableTrashed: '已移入回收站：{{name}}',
  ...
  fieldStandards: {
    title: '字段标准库',
    localHint:
      '标准库保存在当前浏览器，可导出分享或备份。字段引用随表保存和同步；其他设备需导入同一份标准库。',
    name: '业务名称',
    unit: '业务单位',
    description: '业务定义',
    editorHint: '定义一个标准字段。修改标准只更新偏差提示，不会自动覆盖引用它的表。',
    cancel: '取消',
    save: '保存标准',
    operationFailed: '操作失败，请检查文件格式或浏览器存储后重试。',
    retry: '重试',
    create: '新建标准',
    export: '导出标准库',
    import: '导入标准库',
    importPreview:
      '将导入 {{count}} 个标准，其中 {{conflicts}} 个同 ID 标准会被覆盖。现有表字段不会自动更新。',
    confirmImport: '确认合并导入',
    search: '搜索名称、字段或业务定义',
    loading: '正在加载…',
    empty: '暂无匹配标准。可以新建，或从当前字段创建。',
    target: '当前表字段',
    selectField: '选择要引用标准的字段',
    fromField: '从此字段创建标准',
    noDescription: '尚未填写业务定义',
    property: '属性',
    current: '当前值',
    expected: '标准值',
    nameConflict: '当前表已有同名字段，不能覆盖或添加。',
    apply: '应用标准并关联',
    linkOnly: '仅关联，不修改字段',
    append: '新增标准字段',
    edit: '编辑标准',
    applyHint:
      '应用标准会覆盖所选字段的上述属性。请核对高亮差异；保存表和执行数据库变更仍需单独操作。',
    delete: '删除此标准…',
    deleteHint: '删除后保留表字段与引用标识，并显示标准缺失。重新导入同 ID 标准可以恢复关联。',
    confirmDelete: '确认删除标准',
    references: '当前表的标准引用',
    noReferences: '当前表尚未引用标准。',
    missing: '标准缺失',
    drift: '{{count}} 项偏差',
    matches: '符合标准',
    inspect: '查看差异',
    ...
    scan: '检查已保存表',
    scanHint:
      '检查当前工作区已保存的表，不包含未保存草稿。同名但未关联的字段也会列出供核对。结果是本次检查的快照；修改表后请重新检查。',
    scanCount: '找到 {{count}} 个引用或同名字段。',
    unlinked: '尚未关联',
    properties: {
      fieldName: '字段名',
      fieldType: '类型',

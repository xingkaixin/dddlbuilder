...
- 已完成：
  ...
  - 首次登录后匿名工作区迁移提示弹层
  - 迁移成功 / 失败 toast 与重试入口
  - `useAuthSession()` 内统一加载余额并提供 `refreshCredits()`
  - AI generate / review / explain 全部接入登录门禁、余额不足分流与登录引导
  - 匿名用户点击 AI 时打开登录引导，不再返回模糊错误
  - 余额不足与服务异常已映射为不同文案
- 尚未完成：
  - 独立用户中心页
...
- [x] 用户能明确知道自己是否登录
- [x] 用户能看到当前余额或明确的余额加载异常
- [x] 匿名用户看到的是“可注册解锁”，不是模糊报错
- [x] 余额不足与服务异常提示可区分
- [x] 迁移状态对用户可见且可重试
